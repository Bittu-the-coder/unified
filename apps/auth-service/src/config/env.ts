...
if (process.env.NODE_ENV === 'production') {
  if (!process.env.MONGODB_URI || parsed.MONGODB_URI.includes('localhost')) {
    throw new Error('auth-service: MONGODB_URI must be set to a remote database in production');
  }
  if (!process.env.CLIENT_ORIGIN) {
    throw new Error('auth-service: CLIENT_ORIGIN is required in production');
  }
}
