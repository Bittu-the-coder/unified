...
const ensureBootstrap = () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (!rawMongoUri || rawMongoUri.includes('localhost')) {
      throw new Error('productivity-service: set MONGODB_URI to a remote DB in Production env vars');
    }
    if (!rawClientOrigin) {
      throw new Error('productivity-service: set CLIENT_ORIGIN in Production env vars');
    }
  }
